...
function find_first_collision_simd_threaded(ps, vs, rs)
    N = 8
    V = Vec{N,Float64}
    VI = Vec{N,Int}
    ...
    ivr, ir = @inline range_chunks(i_range, N)

    Threads.@threads for id in 1:nth
        @inbounds for i in ivr[id:nth:end]
            iv = VecRange{N}(i)
            pxi = ps[iv, 1]
            pyi = ps[iv, 2]
            vxi = vs[iv, 1]
            ...
            if min_t_local > 0 && !(min_t[id] < min_t_local)
                min_inds[id] = (min_i, min_j)
                min_t[id] = min_t_local
            end
        end
    end
    ...
    @inbounds for (inds, t) in zip(min_inds, min_t)
        if t > 0 && !(min_t_s < t)
            min_inds_s = inds
            min_t_s = t
        end
